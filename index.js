...
client.on("messageReactionAdd", async (message, emoji, userID) => {
    ...
    let starboardChannelID = guild.channels.find(channel => channel.name === starboardChannelName);
    if (starboardChannelID == null) {
        message.channel.createMessage("This server does not have a starboard channel");
        return;
    }

    guild.channels.get(starboard).createMessage({content: channel.mention, embed: embed.render()});
});
